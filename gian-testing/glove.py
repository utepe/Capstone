...
class WBA_Glove:
    ...
    def calibrate(self):
        ...
        for finger in fingers:
            self.pip_joints["calibration"][finger][2] = self.linear_fit(90, 0, self.pip_joints["calibration"][finger][0],
    ...
    def update_MCP_extrema(self):
        for finger in fingers:
            ...
            self.pip_joints["mcp_adjustment"][finger][0] = max(self.pip_joints["mcp_adjustment"][finger][0], 
                                                               self.pip_joints["current_avg"][finger])
            self.pip_joints["mcp_adjustment"][finger][1] = min(self.pip_joints["mcp_adjustment"][finger][1], 
                                                               self.pip_joints["current_avg"][finger])
            
    def update_PIP_extrema(self):
        for finger in fingers:
            self.pip_joints["calibration"][finger][0] = max(self.pip_joints["calibration"][finger][0], 
                                                            self.pip_joints["current_avg"][finger])
